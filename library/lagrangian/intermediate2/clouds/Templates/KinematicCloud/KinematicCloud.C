...
:
    ...
    U_(c.U_),
    mu_(c.mu_),
    alpha1_(c.alpha1_),	//-RM
    alpha1Su_(c.alpha1Su_),	//-RM
    g_(c.g_),
...
//-RM: Set sources to zero. Add source terms here to reset
// The code is called at the beginning of evolve->solve->evolveCloud.
...
void Foam::KinematicCloud<CloudType>::resetSourceTerms()
{
    UTrans().field() = Zero;
    UCoeff().field() = 0.0;
    this->alpha1Su() = 0.0;	//Explicit source for carrier alpha
}
