...
)
{
    ...
    // volScalarField mapAlpha (cloud.pMesh(), 0.0);	//-RM
	...
	//-RM: VOF based deletion

	{
	    volScalarField& vof = cloud.pMesh().objectRegistry::template lookupObjectRef<volScalarField>("alpha.air");
	    label cell = p.cell();
	    scalar alphaCell = vof[cell];

	    point finalP = start + s + 0.5*d_*s/(mag(s)+ROOTVSMALL);
	    label cellFinal = cloud.pMesh().findCell(finalP);
	    scalar tolerance = 0.1;


	    if(cellFinal == -1)
	    {
		if(vof[cell]>tolerance)
		{
...
//Added related to deleteParticle
    //If density changes to gas.
/*
    if (td.rhoc() < scalar(2))
      {
        Info<< "Marked particle by density at "<< p.position()<<endl;
        ttd.keepParticle = false;
      }
*/
    //LPTtoVOF
    //#include "LPTtoVOF.H"	//runs for keepParticle = false.
    return ttd.keepParticle;
}
