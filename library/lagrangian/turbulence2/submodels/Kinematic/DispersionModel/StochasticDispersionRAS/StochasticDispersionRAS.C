...
)
{
    ...
    scalar te = cps*pow(k, 1.5)/epsilon/(UrelMag + small);	//low

    //calculate crossing time
    scalar tcross = great;

    dictionary& customDict = this->owner().db().objectRegistry::template lookupObjectRef<IOdictionary>("values");
    word tMethod(customDict.lookup("tMethod"));
    word dirMethod(customDict.lookup("dirMethod"));

    if(tMethod=="low")
    {
    	te = cps*pow(k, 1.5)/epsilon/(UrelMag + small);	//low
    }
    else if(tMethod=="med")
    {
	te = 0.3*k/epsilon;
    }
    else if(tMethod=="high")
    {
	te = k/epsilon;
    }


    const scalar tTurbLoc = min(te,tcross);
    ...
    if (dt < tTurbLoc)
    {
	  ...
	  else if(dirMethod=="fluent")
	  {
		const vector dir(rnd.scalarNormal(), rnd.scalarNormal(), rnd.scalarNormal());
		UTurb = sigma*dir/(mag(dir)+ROOTVSMALL);
	  }
        }
    }
    else
